...
def _fetch_external(path: str) -> bytes:
    """Récupère un fichier depuis l'hébergement externe (R2 puis URL publique) — utile quand le
    disque local est éphémère (ex. Render gratuit) et a été vidé après une mise en veille."""
    cfg = _assets_cache
    if not cfg.get("enabled"):
        raise FileNotFoundError(path)
    if (cfg.get("provider") or "ftp") == "r2" and cfg.get("r2_bucket") and cfg.get("r2_access_key"):
        try:
            obj = _r2_client(cfg).get_object(Bucket=cfg["r2_bucket"], Key=path)
            return obj["Body"].read()
        except Exception as e:
            logger.warning(f"Récupération R2 échouée pour {path}: {e}")
    pub = (cfg.get("public_url") or "").rstrip("/")
    if pub:
        r = requests.get(f"{pub}/{path}", timeout=30)
...
def get_object(path: str):
    if STORAGE_BACKEND == "local":
        ...
        # Disque local vidé (hébergement éphémère) : on restaure depuis R2 / URL publique.
        try:
            data = _fetch_external(path)
            fp.parent.mkdir(parents=True, exist_ok=True)
            fp.write_bytes(data)
            return data, MIME_TYPES.get(ext, "application/octet-stream")
        except Exception as e:
            logger.warning(f"Fichier local absent et restauration externe impossible pour {path}: {e}")
...
_frontend_origin_cache = {"value": None}


async def _remember_frontend_origin(request: Request):
    """Mémorise l'adresse publique de l'interface (frontend) pour les liens envoyés par courriel."""
    origin = (request.headers.get("x-app-origin") or request.headers.get("origin") or "").strip().rstrip("/")
    if not origin.startswith("http"):
        ref = request.headers.get("referer") or ""
        m = re.match(r"^(https?://[^/]+)", ref)
        origin = m.group(1) if m else ""
    if origin and origin != _frontend_origin_cache["value"]:
        _frontend_origin_cache["value"] = origin
        await db.config.update_one({"key": "frontend"}, {"$set": {"origin": origin}}, upsert=True)


async def _frontend_url() -> str:
    if FRONTEND_URL_ENV:
        return FRONTEND_URL_ENV
    doc = await db.config.find_one({"key": "frontend"}) or {}
    return (doc.get("origin") or BACKEND_PUBLIC_URL or "").rstrip("/")
...
async def login(data: LoginInput, request: Request):
    ...
    await _remember_frontend_origin(request)
...
async def update_settings(data: dict, admin: dict = Depends(require_perm("brand", "setup"))):
    # Mise à jour partielle : seules les clés envoyées sont écrasées, pour ne jamais effacer
    # les changements enregistrés entre-temps par un autre utilisateur (ex. bannières/GIF).
    current = await load_settings()
    incoming = {k: v for k, v in (data or {}).items() if k in CompanySettings.model_fields}
    merged = CompanySettings(**{**current, **incoming}).model_dump()
    payload = {k: merged[k] for k in incoming}
...
async def generate_gif(data: GifGenerateInput, admin: dict = Depends(require_perm("gif", "setup"))):
    ...
    for src in data.images:
        try:
            content, _ = await asyncio.to_thread(get_object, src)
...
def build_signature_html(user, s, include_style=True):
    ...
    if banner["gif_url"]:
        ...
        if link and user.get("id"):
            track = f'{BACKEND_PUBLIC_URL}/api/t/{_esc(user["id"])}'
...
EXO_DISCLAIMER_MAX = 5000  # limite Exchange pour ApplyHtmlDisclaimerText


def _compact_html(html: str) -> str:
    """Réduit le HTML (rendu quasi identique) pour tenir sous la limite d'Exchange."""
    html = re.sub(r'\s+class="[^"]*"', "", html)
    for stack in _FONT_STACKS.values():
        html = html.replace(f"font-family:{stack}", f"font-family:{stack.split(',')[0]}")
    html = html.replace("font-family:Arial,Helvetica,sans-serif", "font-family:Arial")
    html = html.replace("font-family:Arial, Helvetica, sans-serif", "font-family:Arial")
    html = html.replace("font-weight:400;", "").replace(' target="_blank"', "")
    html = html.replace('<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">',
                        '<table cellpadding="0" cellspacing="0" border="0">')
    # Les <img> portent déjà width/height en attributs : on retire les doublons CSS.
    def _img(m):
        tag = m.group(0)
        tag = re.sub(r"(?:display:inline-block;|width:\d+px;|height:\d+px;|max-width:100%;)", "", tag)
        return re.sub(r' alt="[^"]*"', ' alt=""', tag)
    html = re.sub(r"<img\b[^>]*>", _img, html)
    return html


def build_transport_rule_html(user, s):
    """HTML pour règle de flux Exchange : compact et garanti < 5000 caractères.
    Dégrade progressivement (disclaimer en texte brut abrégé → sans disclaimer → sans réseaux sociaux)."""
    lead = '<div style="height:18px;line-height:18px;">&nbsp;</div>'
    budget = EXO_DISCLAIMER_MAX - len(lead) - 20
    dropped = []

    def _build(cfg):
        return _compact_html(build_signature_html(user, cfg, include_style=False))

    html = _build(s)
    if len(html) > budget and s.get("disclaimer"):
        plain = re.sub(r"\s+", " ", re.sub(r"<[^>]+>", " ", s.get("disclaimer") or "")).strip()
        s = {**s, "disclaimer": plain}
        html = _build(s)
        over = len(html) - budget
        if over > 0:
            keep = len(plain) - over - 4
            if keep >= 60:
                s["disclaimer"] = plain[:keep].rstrip() + "…"
                dropped.append("mention légale abrégée")
            else:
                s["disclaimer"] = ""
                dropped.append("mention légale retirée")
            html = _build(s)
    if len(html) > budget and s.get("social"):
        s = {**s, "social": {}}
        html = _build(s)
        dropped.append("réseaux sociaux retirés")
    if len(html) > budget and s.get("disclaimer"):
        s = {**s, "disclaimer": ""}
        html = _build(s)
        dropped.append("mention légale retirée")
    return lead + html, dropped
...
def _email_body(user, signature_html, link):
    name = user.get("name", "")
    return f"""
...
async def _send_signature_to(user: dict, settings: dict):
    ...
    link = f"{await _frontend_url()}/app/signature"
    try:
        await _send_email(user["email"], "Votre signature courriel officielle",
                          _email_body(user, sig, link), attachment_html=_signature_file(sig))
...
# ---------------------------------------------------------------------------
# Signature d'un employé : HTML prêt à copier + lien public (sans connexion)
# ---------------------------------------------------------------------------
async def _ensure_sig_token(emp: dict) -> str:
    tok = emp.get("sig_token")
    if not tok:
        tok = secrets.token_urlsafe(24)
        await db.users.update_one({"_id": emp["_id"]}, {"$set": {"sig_token": tok}})
    return tok


async def _signature_payload(emp: dict) -> dict:
    emp["id"] = str(emp["_id"])
    settings = await load_settings()
    tok = await _ensure_sig_token(emp)
    slug = re.sub(r"[^a-z0-9]+", "-", (emp.get("name") or "signature").lower()).strip("-") or "signature"
    return {"html": build_signature_html(emp, settings),
            "public_url": f"{BACKEND_PUBLIC_URL}/api/sig/{tok}",
            "app_url": f"{await _frontend_url()}/app/signature",
            "filename": f"signature-{slug}.html"}


@api_router.get("/employees/{emp_id}/signature")
async def employee_signature(emp_id: str, admin: dict = Depends(require_perm("employees", "email", "m365", "deploy"))):
    emp = await db.users.find_one({"_id": _oid(emp_id)})
    if not emp:
        raise HTTPException(status_code=404, detail="Employé introuvable")
    return await _signature_payload(emp)


@api_router.get("/auth/me/signature")
async def my_signature(user: dict = Depends(get_current_user)):
    emp = await db.users.find_one({"_id": ObjectId(user["id"])})
    return await _signature_payload(emp)


@app.get("/api/sig/{token}")
async def public_signature_page(token: str):
    emp = await db.users.find_one({"sig_token": token}) if token else None
    if not emp:
        raise HTTPException(status_code=404, detail="Signature introuvable")
    emp["id"] = str(emp["_id"])
    settings = await load_settings()
    sig = build_signature_html(emp, settings)
    name = _esc(emp.get("name") or emp.get("email"))
...
.card{{background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:28px;box-shadow:0 8px 30px rgba(15,23,42,.06)}}
h1{{font-size:20px;margin:0 0 6px}} p.sub{{margin:0 0 20px;color:#64748b;font-size:14px}}
.sig{{border:1px dashed #cbd5e1;border-radius:10px;padding:18px;background:#fff;overflow:auto}}
.actions{{display:flex;gap:10px;flex-wrap:wrap;margin-top:18px}}
button,a.btn{{cursor:pointer;border:0;border-radius:999px;padding:11px 18px;font-weight:700;font-size:14px;text-decoration:none;display:inline-block}}
.primary{{background:#2563eb;color:#fff}} .ghost{{background:#e2e8f0;color:#0f172a}}
.ok{{color:#059669;font-size:13px;margin-top:10px;min-height:18px}}
ol{{font-size:14px;color:#334155;line-height:1.6}}
</style></head><body><div class="wrap"><div class="card">
<h1>Signature courriel de {name}</h1>
<p class="sub">Cliquez « Copier la signature », puis collez-la (Ctrl+V) dans Outlook.</p>
<div class="sig" id="sig">{sig}</div>
<div class="actions">
<button class="primary" id="copy" data-testid="public-copy-signature">Copier la signature</button>
<button class="ghost" id="dl" data-testid="public-download-signature">Télécharger .html</button>
</div>
<div class="ok" id="ok"></div>
<ol><li>Outlook : Fichier → Options → Courrier → <b>Signatures…</b></li>
<li>Créez une nouvelle signature, cliquez dans la zone d'édition puis collez (Ctrl+V).</li>
<li>Choisissez-la comme signature par défaut pour les nouveaux messages et les réponses.</li></ol>
</div></div>
<script>
const sig=document.getElementById('sig');const ok=document.getElementById('ok');
document.getElementById('copy').onclick=async()=>{{
  const html=sig.innerHTML;
  try{{await navigator.clipboard.write([new ClipboardItem({{'text/html':new Blob([html],{{type:'text/html'}}),'text/plain':new Blob([sig.innerText],{{type:'text/plain'}})}})]);}}
  catch(e){{const r=document.createRange();r.selectNodeContents(sig);const s=getSelection();s.removeAllRanges();s.addRange(r);document.execCommand('copy');s.removeAllRanges();}}
  ok.textContent='Signature copiée ! Collez-la dans Outlook (Ctrl+V).';
}};
document.getElementById('dl').onclick=()=>{{
  const full='<!DOCTYPE html><html><head><meta charset="utf-8"></head><body>'+sig.innerHTML+'</body></html>';
  const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([full],{{type:'text/html'}}));a.download='signature.html';a.click();
}};
</script></body></html>"""
    return Response(content=page, media_type="text/html; charset=utf-8", headers={"Cache-Control": "no-store"})
...
@app.get("/api/t/{uid}")
async def track_short(uid: str):
    """Lien de suivi court (garde la signature sous la limite de 5000 caractères d'Exchange).
    La cible est résolue au clic depuis la bannière de l'employé."""
    user = None
    try:
    ...
    banner = _resolve_banner(user or {}, s)
    target = _norm_url(banner.get("banner_link") or "") or _norm_url(s.get("website") or "") or "https://www.google.com"
    try:
        await db.clicks.insert_one({"user_id": uid, "banner": banner.get("key") or "Défaut", "target": target,
                                    "ts": datetime.now(timezone.utc).isoformat()})
    except Exception as e:
        logger.error(f"click log failed: {e}")
    return RedirectResponse(url=target, status_code=302)
...
# ---------------------------------------------------------------------------
def _ps_signatures_block(users, settings, transport=False):
    lines = ["$signatures = @{}"]
    for u in users:
        u["id"] = str(u["_id"])
        sig = build_transport_rule_html(u, settings)[0] if transport else build_signature_html(u, settings)
...
async def deploy_exchange_script(_auth: dict = Depends(deploy_auth)):
    ...
    block = _ps_signatures_block(users, settings, transport=True)
    script = f"""# =====================================================================
...
def _exo_error(r):
    ...
    try:
        ...
        if isinstance(err, dict):
            # Le message externe est souvent le générique « Invalid Operation » : le vrai motif
            # se trouve dans innererror.internalexception.message (ou innererror.message).
            inner = err.get("innererror") or {}
            internal = inner.get("internalexception") or {}
            details = err.get("details") or []
            base = (internal.get("message") or inner.get("message")
                    or (details[0].get("message") if details and isinstance(details[0], dict) else None))
            outer = err.get("message") or ""
            if not base or base.strip().lower() == "invalid operation":
                base = outer if outer.strip().lower() != "invalid operation" else base
            elif outer and outer.strip().lower() != "invalid operation" and outer not in base:
                base = f"{outer} — {base}"
    ...
    if not base:
        try:
            ...
            base = None
    if base and base.strip().lower() == "invalid operation":
        base = ("Exchange Online a répondu « Invalid Operation » sans autre détail. Causes fréquentes : la boîte "
                "n'existe pas dans Exchange (alias ou boîte partagée), ou une règle de flux portant ce nom est verrouillée.")
    if code == 401:
...
async def m365_push(data: M365PushInput, admin: dict = Depends(require_perm("m365", "deploy"))):
    ...
    for email in data.emails:
        ...
        # Version compacte (sans bloc <style>, sans classes) : Exchange limite ApplyHtmlDisclaimerText
        # à 5000 caractères. Espace en tête pour que la signature ne colle pas au texte du message.
        html, dropped = build_transport_rule_html(user, settings)
        if dropped:
            warnings.append({"email": email, "warning": "Signature réduite pour tenir dans la limite Exchange (5000 car.) : " + ", ".join(dropped)})
        ...
        try:
            ...
            if exists.status_code < 300 and (exists.json().get("value") or []):
                ...
                if r.status_code >= 300:
                    # 2e push : Set-TransportRule échoue parfois (« Invalid Operation »). On remplace
                    # proprement la règle : suppression puis recréation.
                    logger.warning(f"Set-TransportRule échoué pour {rule} ({r.status_code}) : {_exo_error(r)} — remplacement")
            ...
            if r.status_code >= 300:
                logger.error(f"Push M365 échoué pour {email} : HTTP {r.status_code} — {r.text[:800]}")
            ...
            failed.append({"email": email, "error": str(e)})
    return {"applied": applied, "failed": failed, "warnings": warnings, "applied_count": len(applied)}
...
async def _referenced_asset_keys() -> set:
    ...
    keys = set()
    marker = "/api/files/"
    acfg = await db.config.find_one({"key": "assets"}) or {}
    pub = (acfg.get("public_url") or "").split("?")[0].rstrip("/")
    ...
    def add(url):
        ...
        elif pub and u.startswith(pub):
            keys.add(u[len(pub):].lstrip("/"))
