...
def build_signature_html(user, s, include_style=True):
    ...
    lines = []
    if phone_str:
        lines.append(f'<tr><td class="sf-text" style="padding:1px 0;{cstyle}"><span class="sf-accent" style="color:{color};font-weight:700;">Tél&nbsp;</span>{phone_str}</td></tr>')
    if direct:
        lines.append(f'<tr><td class="sf-text" style="padding:1px 0;{cstyle}"><span class="sf-accent" style="color:{color};font-weight:700;">Ligne directe&nbsp;</span>{direct}</td></tr>')
    if email:
        lines.append(f'<tr><td class="sf-text" style="padding:1px 0;{cstyle}"><span class="sf-accent" style="color:{color};font-weight:700;">Courriel&nbsp;</span><a href="mailto:{email}" style="color:{ccolor};text-decoration:none;">{email}</a></td></tr>')
    if website:
        lines.append(f'<tr><td class="sf-text" style="padding:1px 0;{cstyle}"><span class="sf-accent" style="color:{color};font-weight:700;">Web&nbsp;</span><a href="{_esc(_norm_url(website))}" style="color:{ccolor};text-decoration:none;">{_esc(website)}</a></td></tr>')
    if address:
    ...
    if layout == "modern":
        ...
        if email:
            parts.append(f'<a href="mailto:{email}" style="color:{ccolor};text-decoration:none;">{email}</a>')
        if website:
            parts.append(f'<a href="{_esc(_norm_url(website))}" style="color:{ccolor};text-decoration:none;">{_esc(website)}</a>')
...
async def test_assets(admin: dict = Depends(require_admin)):
    ...
    def _run():
        try:
            _publish_asset(cfg, f"{APP_NAME}/_sigchamp_test.txt", b"SigChamp connexion OK", "text/plain")
            return {"ok": True}
        except Exception as e:
    ...
    if not res.get("ok"):
        ...
        raise HTTPException(status_code=400, detail=f"Connexion {label} échouée : {res.get('error')}")
