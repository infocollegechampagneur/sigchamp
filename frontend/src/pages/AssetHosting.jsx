...
const empty = {
  enabled: false, public_url: "", ftp_host: "", ftp_port: 21,
...
export default function AssetHosting() {
  ...
  useEffect(() => {
    api.get("/assets/config").then((r) => {
      setForm({ ...empty, ...r.data, ftp_password: "" });
  ...
  return (
    <div className="max-w-3xl" data-testid="asset-hosting-page">
      ...
      <div className="mt-6 rounded-2xl border border-slate-700 bg-[#111827] p-6 space-y-5">
        ...
        <div>
          <Label className="text-slate-300">URL publique (sous-domaine HTTPS)</Label>
          <Input data-testid="hosting-public-url" value={form.public_url} onChange={set("public_url")} placeholder="https://sig.champagneur.qc.ca" className="mt-1.5 bg-slate-800/60 border-slate-700 text-white" />
          <p className="text-xs text-slate-500 mt-1">L'adresse où vos fichiers sont accessibles publiquement (le dossier racine du sous-domaine).</p>
        </div>
