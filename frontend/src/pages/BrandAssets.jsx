...
export default function BrandAssets() {
  ...
  const save = async () => {
    setSaving(true);
    try {
      // On n'envoie que les champs de la charte : les bannières/GIF sont gérés dans « Bannières & GIF ».
      const { gif_images, gif_interval_ms, gif_url, gif_version, department_banners, ...brand } = s;
      const { data } = await api.put("/settings", brand);
      setS(data);
