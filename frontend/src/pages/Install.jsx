...
const METHODS = [
  {
    ...
    prereqs: [
      ...
      "Le code du projet SigChamp copié sur le serveur (git clone ou envoi des fichiers)",
      "Un nom de domaine + certificat HTTPS recommandés en production",
    ],
    steps: [
      { title: "Récupérer le projet", desc: "Placez le code sur le serveur.", code: "git clone <URL_DE_VOTRE_DEPOT> sigchamp\ncd sigchamp" },
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Copier le projet", desc: "Par exemple dans C:\\SigChamp (dossiers backend et frontend).", code: "# C:\\SigChamp\\backend\n# C:\\SigChamp\\frontend" },
      { title: "Préparer le backend", desc: "Environnement virtuel + dépendances.", code: "cd C:\\SigChamp\\backend\npython -m venv venv\n.\\venv\\Scripts\\Activate.ps1\npip install -r requirements.txt" },
      { title: "Créer C:\\SigChamp\\backend\\.env", desc: "Renseignez la configuration :", code: "MONGO_URL=mongodb://localhost:27017\nDB_NAME=sigchamp\nCORS_ORIGINS=*\nJWT_SECRET=<chaine_aleatoire_64_hex>\nADMIN_EMAIL=admin@monentreprise.com\nADMIN_PASSWORD=<mot_de_passe_fort>\nSTORAGE_BACKEND=local\nLOCAL_STORAGE_DIR=C:\\SigChamp\\backend\\storage\nDEPLOY_API_TOKEN=<chaine_aleatoire>\nREACT_APP_BACKEND_URL=https://signatures.monentreprise.com" },
      ...
      { title: "Installer le backend en service (NSSM)", desc: "Pour un démarrage automatique et permanent :", code: "nssm install SigChampBackend \"C:\\SigChamp\\backend\\venv\\Scripts\\python.exe\" \"-m uvicorn server:app --host 0.0.0.0 --port 8001\"\nnssm set SigChampBackend AppDirectory C:\\SigChamp\\backend\nnssm start SigChampBackend" },
      { title: "Construire le frontend", desc: "Créez d'abord C:\\SigChamp\\frontend\\.env avec l'URL du backend, puis build :", code: "cd C:\\SigChamp\\frontend\n# .env :\n#   REACT_APP_BACKEND_URL=https://signatures.monentreprise.com\nyarn install\nyarn build" },
      { title: "Publier sur IIS", desc: "Créez un site pointant vers C:\\SigChamp\\frontend\\build. Ajoutez ce web.config à la racine du build pour React Router :", code: "<configuration>\n  <system.webServer>\n    <rewrite><rules>\n      <rule name=\"SPA\" stopProcessing=\"true\">\n        <match url=\".*\" />\n        <conditions logicalGrouping=\"MatchAll\">\n          <add input=\"{REQUEST_FILENAME}\" matchType=\"IsFile\" negate=\"true\" />\n          <add input=\"{REQUEST_FILENAME}\" matchType=\"IsDirectory\" negate=\"true\" />\n        </conditions>\n        <action type=\"Rewrite\" url=\"/index.html\" />\n      </rule>\n    </rules></rewrite>\n  </system.webServer>\n</configuration>" },
      { title: "Ouvrir le pare-feu", desc: "Autorisez les ports nécessaires.", code: "New-NetFirewallRule -DisplayName \"SigChamp API\" -Direction Inbound -LocalPort 8001 -Protocol TCP -Action Allow\nNew-NetFirewallRule -DisplayName \"HTTP/HTTPS\" -Direction Inbound -LocalPort 80,443 -Protocol TCP -Action Allow" },
    ],
    notes: [
  ...
  },
  {
    ...
    steps: [
      ...
      { title: "Récupérer le projet", desc: "", code: "sudo mkdir -p /opt/sigchamp && sudo chown $USER /opt/sigchamp\ngit clone <URL_DE_VOTRE_DEPOT> /opt/sigchamp\ncd /opt/sigchamp" },
      { title: "Préparer le backend", desc: "", code: "cd /opt/sigchamp/backend\npython3.11 -m venv venv\nsource venv/bin/activate\npip install -r requirements.txt" },
      { title: "Créer /opt/sigchamp/backend/.env", desc: "", code: "MONGO_URL=mongodb://localhost:27017\nDB_NAME=sigchamp\nCORS_ORIGINS=*\nJWT_SECRET=$(openssl rand -hex 32)\nADMIN_EMAIL=admin@monentreprise.com\nADMIN_PASSWORD=<mot_de_passe_fort>\nSTORAGE_BACKEND=local\nLOCAL_STORAGE_DIR=/opt/sigchamp/backend/storage\nDEPLOY_API_TOKEN=$(openssl rand -base64 24)\nREACT_APP_BACKEND_URL=https://signatures.monentreprise.com" },
      { title: "Service systemd du backend", desc: "Créez /etc/systemd/system/sigchamp.service :", code: "[Unit]\nDescription=SigChamp Backend\nAfter=network.target mongod.service\n\n[Service]\nWorkingDirectory=/opt/sigchamp/backend\nEnvironmentFile=/opt/sigchamp/backend/.env\nExecStart=/opt/sigchamp/backend/venv/bin/uvicorn server:app --host 0.0.0.0 --port 8001\nRestart=always\n\n[Install]\nWantedBy=multi-user.target" },
      { title: "Activer le service", desc: "", code: "sudo systemctl daemon-reload\nsudo systemctl enable --now sigchamp\ncurl http://localhost:8001/api/" },
      { title: "Construire le frontend", desc: "", code: "cd /opt/sigchamp/frontend\necho 'REACT_APP_BACKEND_URL=https://signatures.monentreprise.com' > .env\nyarn install\nyarn build\nsudo mkdir -p /var/www/sigchamp\nsudo cp -r build/* /var/www/sigchamp/" },
      { title: "Configurer nginx", desc: "Créez /etc/nginx/sites-available/sigchamp puis activez :", code: "server {\n    listen 80;\n    server_name signatures.monentreprise.com;\n    root /var/www/sigchamp;\n    index index.html;\n    location / { try_files $uri $uri/ /index.html; }\n    location /api/ {\n        proxy_pass http://127.0.0.1:8001;\n        proxy_set_header Host $host;\n        proxy_set_header X-Forwarded-Proto $scheme;\n    }\n}" },
      { title: "Activer le site + HTTPS", desc: "", code: "sudo ln -s /etc/nginx/sites-available/sigchamp /etc/nginx/sites-enabled/\nsudo nginx -t && sudo systemctl reload nginx\nsudo apt install -y certbot python3-certbot-nginx\nsudo certbot --nginx -d signatures.monentreprise.com" },
    ],
    notes: [
      "Sauvegardez /opt/sigchamp/backend/storage (logos + banner.gif).",
      "Mise à jour : git pull, puis yarn build + cp, et sudo systemctl restart sigchamp.",
    ],
  },
  {
    id: "render",
    ...
    steps: [
      { title: "Pousser le code sur GitHub", desc: "", code: "git init\ngit add .\ngit commit -m \"SigChamp\"\ngit branch -M main\ngit remote add origin git@github.com:VOUS/sigchamp.git\ngit push -u origin main" },
      ...
      { title: "Récupérer l'URL du backend puis redéployer", desc: "L'URL du backend n'est connue qu'après sa 1re création. Copiez-la, mettez-la dans REACT_APP_BACKEND_URL (backend ET frontend), puis redéployez le frontend.", code: "# Backend : https://sigchamp-backend.onrender.com\n# Manual Deploy → Deploy latest commit (frontend)" },
    ],
    notes: [
