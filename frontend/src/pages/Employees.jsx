...
import { EmployeeSigMenu } from "@/components/EmployeeSigMenu";
import { useBulkPush } from "@/components/BulkPushBar";
...
export default function Employees() {
  ...
  const toggleAll = () => setSelected(allSelected ? [] : list.map((e) => e.id));
  const pushTargets = selected.length ? list.filter((e) => selected.includes(e.id)) : list;
  const bulk = useBulkPush(pushTargets, list.length, () => api.get("/employees").then((r) => setList(r.data)));
  ...
  const pushSig = async (emp) => {
    ...
    try {
      ...
      if (data.warnings?.length) toast.warning(data.warnings[0].warning, { duration: 12000 });
    ...
    <div className="max-w-6xl mx-auto px-5 lg:px-8 py-8">
      ...
      <div className="rounded-2xl bg-[#111827] border border-slate-800 overflow-hidden sf-fade-up">
        <Table>
          <TableHeader>
            <TableRow className="border-slate-800 hover:bg-transparent">
              <TableHead className="w-10">
                <Checkbox checked={allSelected} onCheckedChange={toggleAll} data-testid="select-all-employees" className="border-slate-500 data-[state=checked]:bg-emerald-600 data-[state=checked]:border-emerald-600" />
              </TableHead>
          ...
          <TableBody>
            {list.map((emp) => (
              <TableRow key={emp.id} className={`border-slate-800 hover:bg-slate-800/40 ${selected.includes(emp.id) ? "bg-emerald-500/5" : ""}`} data-testid={`employee-row-${emp.id}`}>
                <TableCell>
                  <Checkbox checked={selected.includes(emp.id)} onCheckedChange={() => toggleSelect(emp.id)} data-testid={`select-employee-${emp.id}`} className="border-slate-500 data-[state=checked]:bg-emerald-600 data-[state=checked]:border-emerald-600" />
                </TableCell>
                <TableCell>
