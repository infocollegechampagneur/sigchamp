...
import { Upload, Save, Loader2, Trash2, Link2, ExternalLink } from "lucide-react";

function PublicLinkCard() {
  const [url, setUrl] = useState("");
  useEffect(() => { api.get("/auth/me/signature").then((r) => setUrl(r.data.public_url)).catch(() => {}); }, []);
  if (!url) return null;
  const copy = async () => { await navigator.clipboard.writeText(url); toast.success("Lien public copié."); };
  return (
    <div className="rounded-2xl bg-[#111827] border border-slate-800 p-5" data-testid="public-link-card">
      <p className="text-sm font-semibold text-white flex items-center gap-2"><Link2 className="h-4 w-4 text-blue-400" /> Lien public de votre signature</p>
      <p className="text-xs text-slate-500 mt-1">Ouvrez-le depuis n'importe quel appareil (aucune connexion requise) pour copier votre signature.</p>
      <div className="flex gap-2 mt-3">
        <Input readOnly value={url} data-testid="public-link-input" className="bg-slate-800/60 border-slate-700 text-slate-300 text-xs font-mono" />
        <Button variant="outline" onClick={copy} data-testid="button-copy-public-link" className="border-slate-700 bg-slate-800/50 text-slate-200 hover:bg-slate-700 hover:text-white shrink-0">Copier</Button>
        <a href={url} target="_blank" rel="noopener noreferrer" data-testid="link-open-public" className="inline-flex items-center px-3 rounded-md border border-slate-700 bg-slate-800/50 text-slate-200 hover:bg-slate-700"><ExternalLink className="h-4 w-4" /></a>
      </div>
    </div>
  );
}
          ...
          <div className="rounded-2xl bg-[#111827] border border-slate-800 p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <textarea data-testid="input-employee-title" value={form.title} onChange={set("title")} rows={2} placeholder="Un poste par ligne (ex. Directeur des ventes)" className="mt-1.5 w-full rounded-md bg-slate-800/60 border border-slate-700 text-white px-3 py-2 text-sm resize-y" />
              </div>
              <div>
